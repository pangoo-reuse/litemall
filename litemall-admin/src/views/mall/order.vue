<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-date-picker v-model="listQuery.timeArray" type="datetimerange" class="filter-item" range-separator="至"
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" width="250" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['POST /admin/order/delete']" type="danger" size="mini"
                     @click="handleDelete(scope.row)">删除
          </el-button>
          <el-button v-if="scope.row.orderStatus==201" v-permission="['POST /admin/order/ship']" type="primary"
                     size="mini" @click="handleShip(scope.row)">发货
          </el-button>
          <el-button v-if="scope.row.orderStatus==202||scope.row.orderStatus==204"
                     v-permission="['POST /admin/order/refund']" type="primary" size="mini"
    ...
    <el-dialog :visible.sync="shipDialogVisible" title="发货">
      <el-form ref="shipForm" :model="shipForm" status-icon label-position="left" label-width="100px"
        ...
        <el-form-item label="快递编号" prop="shipSn" v-if="shipForm.shipChannel != 'TCS'">
          <el-input v-model="shipForm.shipSn"/>
        </el-form-item>
        <el-form-item label="配送员" prop="deliveryName" v-else>
          <el-autocomplete
            v-model="shipForm.deliveryName"
            popper-class="my-autocomplete"
            placeholder="选择配送员"
            :fetch-suggestions="queryDeliveryList"
            :trigger-on-focus="false"
            @select="handleSelectDelivery"
          >
            <template slot-scope="{ item }" class="inlineBlock">
              <div class="name">{{ item.deliveryName }}</div>
              <span class="addr" style="font-size: 12px;color: #b4b4b4;">{{ item.deliveryRegion }}</span>
            </template>
          </el-autocomplete>
        </el-form-item>
      </el-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            checkPermission,
            queryDeliveryList(queryString, cb) {
                console.log(queryString)
                deliveryList({"deliveryName":queryString}).then(response => {
                    var data = response.data.data.list
                    console.log(data)
                    for (const ex of data) {
                        ex.value = ex.deliveryName // ps:必须为每个对象增加value字段！！因为autocomplete只识别value字段并在下拉列中显示
                    }
                    cb(data)
            ...
            handleSelectDelivery(item) {
                this.delivery = item;
                this.shipForm.deliveryId = item.id;
                this.shipForm.deliveryName = item.deliveryName;
                console.log(item)
            },
            getList() {
